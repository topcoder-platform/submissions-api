  ...
  if (!esClients.client) {
    ...
    if (/.*amazonaws.*/.test(esHost)) {
      ...
      esClients.client = new elasticsearch.Client({
        node: `https://elastic:uLMsx29BQha*CPEyoMJg@localhost:9200`,
      });
    }
  }
...
function setPaginationHeaders(req, res, data) {
  ...
  let fullUrl =
    ...
    "://" +
    req.get("host") +
    req.url.replace(`&page=${data.page}`, "");
...
async function getLegacyChallengeId(challengeId) {
  if (
    ...
    logger.debug(
      `${challengeId} detected as uuid. Fetching legacy challenge id`
    );
    const token = await getM2Mtoken();
    try {
      const response = await request
        .get(`${config.CHALLENGEAPI_V5_URL}/${challengeId}`)
      ...
      logger.debug(
        `Legacy challenge id is ${legacyId} for v5 challenge id ${challengeId}`
      );
      return legacyId;
    } catch (err) {
      logger.error(
        `Error while accessing ${config.CHALLENGEAPI_V5_URL}/${challengeId}`
      );
      throw err;
    }
  }
...
async function getV5ChallengeId(challengeId) {
  if (
    ...
    logger.debug(
      `${challengeId} detected as legacy challenge id. Fetching legacy challenge id`
    );
    const token = await getM2Mtoken();
    try {
      const response = await request
        .get(`${config.CHALLENGEAPI_V5_URL}?legacyId=${challengeId}`)
        .set("Authorization", `Bearer ${token}`)
        .set("Content-Type", "application/json");
      const v5Uuid = _.get(response, "body[0].id");
      logger.debug(
        `V5 challenge id is ${v5Uuid} for legacy challenge id ${challengeId}`
      );
      return v5Uuid;
    } catch (err) {
      logger.error(
        `Error while accessing ${config.CHALLENGEAPI_V5_URL}?legacyId=${challengeId}`
      );
      throw err;
    }
  }
 ...
 */
async function getSubmissionPhaseId(challengeId) {
  let phaseId = null;
  let response;
  challengeId = await getV5ChallengeId(challengeId);

  try {
    logger.info(
      `Calling to challenge API to find submission phase Id for ${challengeId}`
    );
    const token = await getM2Mtoken();
  ...
  if (response) {
    ...
    const checkPoint = _.filter(phases, {
 ...
 * @returns {Promise}
 */
async function checkCreateAccess(authUser, subEntity) {
  let challengeDetails;
  let resources;

  const challengeId = await getV5ChallengeId(subEntity.challengeId);

  // User can only create submission for themselves
  if (authUser.userId !== subEntity.memberId) {
    throw new errors.HttpStatusError(
      403,
      "You are not allowed to submit on behalf of others"
    );
  }
  ...
  try {
    logger.info(
      `Calling to challenge API for fetch phases and winners for ${challengeId}`
    );
    challengeDetails = await request
      .get(`${config.CHALLENGEAPI_V5_URL}/${challengeId}`)
      .set("Authorization", `Bearer ${token}`)
      .set("Content-Type", "application/json");
    logger.info(
      `returned for ${challengeId} with ${JSON.stringify(challengeDetails)}`
    );
  } catch (ex) {
    logger.error(
      `Error while accessing ${config.CHALLENGEAPI_V5_URL}/${challengeId}`
    ...
    throw new errors.HttpStatusError(
      503,
      `Could not fetch details of challenge with id ${challengeId}`
    );
  }

  try {
    resources = await request
      .get(
        `${config.RESOURCEAPI_V5_BASE_URL}/resources?challengeId=${challengeId}`
      )
      .set("Authorization", `Bearer ${token}`)
      .set("Content-Type", "application/json");
  } catch (ex) {
    logger.error(
      `Error while accessing ${config.RESOURCEAPI_V5_BASE_URL}/resources?challengeId=${challengeId}`
    );
    logger.error(ex);
    throw new errors.HttpStatusError(
      503,
      `Could not determine the user's role in the challenge with id ${challengeId}`
    );
  }
  ...
  if (resourceRolesMap == null || _.size(resourceRolesMap) === 0) {
    throw new errors.HttpStatusError(
      503,
      `Could not determine the user's role in the challenge with id ${subEntity.challengeId}`
    );
  }

  if (resources && challengeDetails) {
    const currUserRoles = _.filter(resources.body, {
      memberHandle: authUser.handle,
    });
    ...
    if (reviewers.length !== 0) {
      throw new errors.HttpStatusError(
        400,
        `You cannot create a submission for a challenge while you are a reviewer`
      );
    }

    // Check if the User is assigned as the iterative reviewer for the contest
    const iterativeReviewers = _.filter(currUserRoles, {
      role: "Iterative Reviewer",
    });
    if (iterativeReviewers.length !== 0) {
      throw new errors.HttpStatusError(
        400,
        `You cannot create a submission for a challenge while you are an iterative reviewer`
      );
    }
    ...
    if (submitters.length === 0) {
      throw new errors.HttpStatusError(
        403,
        `Register for the contest before you can submit`
      );
    }

    const submissionPhaseId = await getSubmissionPhaseId(subEntity.challengeId);

    if (submissionPhaseId == null) {
      throw new errors.HttpStatusError(
        403,
        "You cannot create a submission in the current phase"
      );
    }
    ...
    if (currPhase[0].name === "Final Fix" || currPhase[0].name === "Approval") {
      ...
      if (userSubmission.total === 0) {
        throw new errors.HttpStatusError(
          403,
          "You are not expected to create a submission in the current phase"
        );
      }
    }
  } else {
    ...
    throw new errors.HttpStatusError(
      503,
      `Not all information could be fetched about challenge with id ${subEntity.challengeId}`
    );
  }
}
...
async function checkGetAccess(authUser, submission) {
  ...
  const token = await getM2Mtoken();
  const challengeId = await getV5ChallengeId(submission.challengeId);
  ...
  if (resources && challengeDetails) {
    ...
    if (subTrack === "FIRST_2_FINISH") {
      const iterativeReviewer = _.filter(currUserRoles, {
        role: "Iterative Reviewer",
      });
      ...
      if (iterativeReviewer.length !== 0) {
        ...
        // In F2F, Member cannot access other memeber submissions
        throw new errors.HttpStatusError(
          403,
          "You cannot access other member submission"
        );
      }
    } else {
      // For other sub tracks, check if the Review / Screening phase is not scheduled
      ...
      if (screener.length !== 0 || reviewer.length !== 0) {
        const screeningPhaseStatus = getPhaseStatus(
          "Screening",
          challengeDetails.body
        );
        const reviewPhaseStatus = getPhaseStatus(
          "Review",
          challengeDetails.body
        );
        ...
        if (
          screeningPhaseStatus === "Scheduled" &&
          reviewPhaseStatus === "Scheduled"
        ) {
          throw new errors.HttpStatusError(
            403,
            "You can access the submission only when Screening / Review is open"
          );
        }
      } else {
        const appealsResponseStatus = getPhaseStatus(
          "Appeals Response",
          challengeDetails.body
        );
        ...
        if (
          appealsResponseStatus !== "Closed" &&
          appealsResponseStatus !== "Invalid"
        ) {
          throw new errors.HttpStatusError(
            403,
            "You cannot access other submissions before the end of Appeals Response phase"
          );
        } else {
          const userSubmission = await fetchFromES(
            {
          ...
          if (userSubmission.total === 0) {
            throw new errors.HttpStatusError(
              403,
              "You did not submit to the challenge!"
            );
          }
          ...
          if (
            reqSubmission.reviewSummation &&
            reqSubmission.reviewSummation[0].isPassing
          ) {
            return true;
          } else {
            throw new errors.HttpStatusError(
              403,
              "You should have passed the review to access other member submissions!"
            );
          }
        }
      }
    ...
    throw new errors.HttpStatusError(
      503,
      `Not all information could be fetched about challenge with id ${submission.challengeId}`
    );
  }
}
...
async function checkReviewGetAccess(authUser, submission) {
  ...
  if (resources && challengeDetails) {
    ...
    const passedRoles = currUserRoles.filter((a) =>
      validRoles.includes(a.role)
    );
    ...
    if (subTrack === "DEVELOP_MARATHON_MATCH") {
      ...
      if (appealsResponseStatus !== "Closed") {
        throw new errors.HttpStatusError(
          403,
          "You cannot access the review before the end of the Appeals Response phase"
        );
      }
...
function cleanseReviews(reviews, authUser) {
  ...
  if (!authUser.scopes) {
    ...
    const admin = _.filter(
      authUser.roles,
      (role) => role.toLowerCase() === "Administrator".toLowerCase()
    );
    const copilot = _.filter(
      authUser.roles,
      (role) => role.toLowerCase() === "Copilot".toLowerCase()
    );
...
async function getRoleIdToRoleNameMap() {
  ...
  const token = await getM2Mtoken();
  try {
    resourceRoles = await request
      .get(`${config.RESOURCEAPI_V5_BASE_URL}/resource-roles`)
      .set("Authorization", `Bearer ${token}`)
      .set("Content-Type", "application/json");
  } catch (ex) {
    logger.error(
      `Error while accessing ${config.RESOURCEAPI_V5_BASE_URL}/resource-roles`
    );
    logger.error(ex);
    resourceRoles = null;
  }
...
async function getLatestChallenges(page) {
  ...
  try {
    ...
    const challenges = _.map(
      _.filter(_.get(response, "body"), "legacyId"),
      (c) => _.pick(c, "id", "legacyId")
    );
    logger.debug(
      `Fetched ${challenges.length} challenges in this iteration. More may follow...`
    );
...
function getLegacyScoreCardId(scoreCardId) {
  if (
    ...
    logger.debug(
      `${scoreCardId} detected as uuid. Converting to legacy scorecard id`
    );

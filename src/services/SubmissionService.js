...
const { originator, mimeType, fileType, events } =
...
async function _getSubmission(submissionId, fetchReview = true) {
  ...
  if (fetchReview) {
    ...
    const reviewSummation = await _getReviewSummationsForSubmission(
      submissionId
    );
...
async function getSubmission(authUser, submissionId) {
  ...
  if (response.total === 0) {
    // CWD-- not in ES yet maybe? let's grab from the DB
    ...
    if (!_.get(submissionRecord, "id", null)) {
      logger.info(
        `getSubmission: submissionId not found in ES nor the DB: ${submissionId}`
      );
    ...
    if (!submissionRecord.review || submissionRecord.review.length < 1) {
      logger.info(
        `submission ${submissionId} from ES has no reviews. Double checking the db`
      );
      const review = await _getReviewsForSubmission(submissionId);
      logger.info(`${review}`);
    ...
    if (
      !submissionRecord.reviewSummation ||
      submissionRecord.reviewSummation.length < 1
    ) {
      logger.info(
        `submission ${submissionId} from ES has no reviewSummations. Double checking the db`
      );
      const reviewSummation = await _getReviewSummationsForSubmission(
        submissionId
      );
  ...
  if (!submissionRecord) {
    // CWD-- couldn't find it in ES nor the DB
    logger.info(
      `getSubmission: submissionId not found in ES nor DB so throwing 404: ${submissionId}`
    );
    throw new errors.HttpStatusError(
      404,
      `Submission with ID = ${submissionId} is not found`
    );
  }

  logger.info("Check User access before returning the submission");
  if (
    _.intersection(authUser.roles, ["Administrator", "administrator"])
      .length === 0 &&
    !authUser.scopes
  ) {
  ...
  submissionRecord.review = helper.cleanseReviews(
    submissionRecord.review,
    authUser
  );

  // Return the retrieved submission
  logger.info(
    `getSubmission: returning data for submissionId: ${submissionId}`
  );
...
async function listSubmissions(authUser, query) {
  ...
  if (query["review.scoreCardId"]) {
    query["review.scoreCardId"] = helper.getLegacyScoreCardId(
      query["review.scoreCardId"]
    );

    if (!query["review.scoreCardId"]) {
      throw new errors.HttpStatusError(
        400,
        "Legacy scorecard id not found for the provided v5 scorecard id"
      );
    }
  }
  ...
  logger.info(
    `listSubmissions: returning ${
      data.length
    } submissions for query: ${JSON.stringify(query)}`
  );
...
listSubmissionsQuerySchema.sortBy = joi
  .string()
  .valid(
    _.difference(Object.keys(listSubmissionsQuerySchema), [
      "page",
      "perPage",
      "orderBy",
    ])
...
listSubmissions.schema = {
  ...
  query: joi
    .object()
    .keys(listSubmissionsQuerySchema)
    .with("orderBy", "sortBy"),
};
...
async function createSubmission(authUser, files, entity) {
  ...
  if (files && entity.url) {
    logger.info(
      "Cannot create submission. Neither file nor url to upload has been passed"
    );
    throw new errors.HttpStatusError(
      400,
      "Either file to be uploaded or URL should be present"
    );
  }

  if (!files && !entity.url) {
    logger.info(
      "Cannot create submission. Ambiguous parameters. Both file and url have been provided. Unsure which to use"
    );
    throw new errors.HttpStatusError(
      400,
      "Either file to be uploaded or URL should be present"
  ...
  if (files && files.submission) {
    ...
    if (pFileType !== uFileType) {
      logger.info(
        "Actual file type of the file does not match the file type attribute in the request"
      );
      throw new errors.HttpStatusError(
        400,
        "fileType parameter doesn't match the type of the uploaded file"
      );
    }
    const file = await _uploadToS3(
      files.submission,
      `${submissionId}.${uFileType}`
    );
    url = file.Location;
  } else if (files) {
    throw new errors.HttpStatusError(
      400,
      'The file should be uploaded under the "submission" attribute'
    );
  }
  ...
  if (entity.submissionPhaseId) {
    ...
    item.submissionPhaseId = await helper.getSubmissionPhaseId(
      entity.challengeId
    );
  }
  ...
  if (
    ...
    if (entity.submittedDate) {
      throw new errors.HttpStatusError(
        403,
        "You are not allowed to set the `submittedDate` attribute on a submission"
      );
    }
  } else {
    logger.info(
      `No need to call checkCreateAccess for ${JSON.stringify(authUser)}`
    );
  }
  ...
  logger.info(JSON.stringify(record.Item, null, 2));
  await submissionDomain.create(record.Item);
  ...
  if (files && files.submission) {
    ...
    // If the file URL is provided, handle accordingly
  ...
  try {
    ...
    logger.error(`Error posting to bus api ${e}`);
  }
...
async function deleteSubmission(authUser, submissionId) {
  ...
  if (
    _.intersection(authUser.roles, ["Administrator", "administrator"])
      .length === 0 &&
    exist.memberId !== authUser.userId
  ) {
    throw new errors.HttpStatusError(
      403,
